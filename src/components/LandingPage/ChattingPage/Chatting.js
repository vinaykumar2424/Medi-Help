...
const Chatting = () => {
      ...
      uploadTask.on(
        ...
        () => {
          getDownloadURL(uploadTask.snapshot.ref).then(async (downloadURL) => {
            await updateDoc(doc(db, "DoctorPatientChats", data.chatId), {
              messages: arrayUnion({
                id: uuid(),
                text,
  ...
  useEffect(() => {
    const getmsg = () => {
      const unSub = onSnapshot(doc(db, "DoctorPatientChats", data.chatId), (doc) => {
        if (doc.exists()) {
          setIsLoading(false)
        ...
        <div className="Chatting">
          <div className="chattingWindow" ref={chatWindowRef}>
            {messages.map((message) => (
              <div className={`${currentUser.uid == message.senderId ? 'chatMessage' : 'chatMessageElse'}`} key={message.id}>
                <div className={`${currentUser.uid == message.senderId ? "roleHeading" : "roleHeadingElse"}`}>{`${currentUser.uid == message.senderId ? `You` : 'Doctor'}`}</div>
                <div className="chatmsgValue">
                  <div>{message.text}</div>
                  {message.image && <img src={message.image} alt='' className='imageMessage' />}
                </div>
